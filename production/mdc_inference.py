...
@jit
def logpm1_powerlaw(m1,m_min,m_max,alpha):
    pm1 = mass**(-alpha)
    # jax.debug.print('{}', pm1)
...
@jit
def logpm1_powerlaw_powerlaw(m1,m_min_1,m_max_1,alpha_1,mu,sigma,f1):
    p1 = jnp.exp(logpm1_powerlaw(m1,m_min_1,m_max_1,alpha_1))
...
@jit
def log_p_pop_pl_pl(m1,m2,z,gamma, m_min_1,m_max_1,alpha_1,beta,mu,sigma,f1):
    # start_time = time.time()
    log_dNdm1 = logpm1_powerlaw_powerlaw(m1,m_min_1,m_max_1,alpha_1,mu,sigma,f1)
    log_dNdm2 = logpm1_powerlaw_powerlaw(m2,m_min_1,m_max_1,alpha_1,mu,sigma,f1)
    q = m2/m1
...
M = chirp_mass(m1detsels, m2detsels)
lm = jnp.log(M)

X = np.column_stack([np.asarray(lm), np.asarray(lq), np.asarray(dLsels)])
...
# # 3. Bulletproof fix: Force the very last element to absorb any tiny floating-point remainder
# prob[-1] = 1.0 - np.sum(prob[:-1])

# # 4. Clip to ensure no tiny negative values were created, just in case
...
lM_sels, lq_sels, dL_sels = X_resam[:, 0], X_resam[:, 1], X_resam[:, 2]
M_sels = np.exp(lM_sels)

from scipy.special import expit

# q_sels = expit(lq_sels)
# m2_sels = m1_sels * q_sels
m1_sels, m2_sels = inverse_transform(M_sels, lq_sels)
...
lM_sam, lq_sam, dL_sam = new_x[0][:, 0], new_x[0][:, 1], new_x[0][:, 2]
M_sam = np.exp(lM_sam)
m1_sam, m2_sam = inverse_transform(M_sam, lq_sam)
...
lM_sam = np.log(chirp_mass(m1_sam, m2_sam))
X_sam = np.column_stack([np.asarray(lM_sam), np.asarray(lq_sam), np.asarray(dL_sam)])
...
@jit
def likelihood_method_1(gamma,m_min,m_max,alpha,beta,mu,sigma,f1):
    log_det_weights = log_p_pop_pl_pl(m1sels,m2sels,zsels,gamma,m_min,m_max,alpha,beta,mu,sigma,f1)
    ...
    log_weights = log_p_pop_pl_pl(m1,m2,z,gamma,m_min,m_max,alpha,beta,mu,sigma,f1)
...
def loglike_method_1(coord):
    ...
    # ll, Neff = likelihood_lvk_met1(gamma, m_min, m_max, alpha, beta, mu, sigma, f1)
...
def sample_from_q_batched(key, batch_size, w_q, mu_all, L_prec):
    """
        Returns a generator yielding (key, x_batch) with x_batch.shape = (B,3)
            """
    while True:
...
def prepare_guided_is_cache_batched(
    ...
    """
    Batched version of prepare_guided_is_cache.

    Returns the same cache dict but built safely in batches.
    """
    ...
    while n_kept < N_samples:
        key, x = next(sampler)              # (B,3)
        x_sorted = sort_masses(x)           # (B,3)
        ...
        x_s = x_sorted[mask]                # (B',3)

        # ---- log q_ord ----
        lq = log_gmm_symmetrized(
            x_s, w_q, mu_all, Prec_all, ld_all
        )                                   # (B',)

        # ---- per-event log posterior ----
        log_pe_list = []
        for w, mu, Prec, ld in zip(event_wts, event_mus, event_precs, event_logdets):
            log_pe_list.append(
                log_gmm_symmetrized(x_s, w, mu, Prec, ld)
            )                               # each (B',)

        log_pe = jnp.stack(log_pe_list, axis=0)  # (E,B')
...
cache = prepare_guided_is_cache_batched(
    ...
    event_wts=wts,
    event_mus=mus,
    event_precs=precisions,
    event_logdets=logdets,
    support_fn=support_mask,
)
...
def logpdf_sum_chunked(
    X, wts, mus, precisions, logdets, chunk_size=40000
):
    """
    Memory-safe version: chunks over N
    """
    E, K = wts.shape
    N, D = X.shape
    ...
    def chunk_fn(X_chunk):
        ...
        diffs = X_chunk[:, None, None, :] - mus[None, :, :, :]   # (Nc,E,K,D)
        quad  = jnp.einsum(
            'nekd,ekdf,nekf->nek',
            diffs, precisions, diffs
        )                                                        # (Nc,E,K)

        log_comp = -0.5 * (
            D*jnp.log(2*jnp.pi) + logdets[None,:,:] + quad
        )                                                        # (Nc,E,K)

        logw = jnp.log(jnp.clip(wts, 1e-12, 1.0))[None,:,:]
        logp = jsp.special.logsumexp(logw + log_comp, axis=-1)   # (Nc,E)
...
@jit
def likelihood_method_3(gamma,m_min,m_max,alpha,beta,mu,sigma,f1):
    ...
    # ll = 0
    # start_time = time.time()
    log_pop = log_p_pop_pl_pl(m1s_pop,m2s_pop,zs_pop,gamma,m_min,m_max,alpha,beta,mu,sigma,f1)
...
def loglike_method_3(coord):
    ...
    # ll, Neff = likelihood_lvk_met3(gamma, m_min, m_max, alpha, beta, mu, sigma, f1)
